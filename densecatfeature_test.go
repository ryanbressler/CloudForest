...
func TestCatFeature(t *testing.T) {
	...
	_, split, _, _ := fm.BestSplitter(f, &cases, &[]int{0}, 1, nil, 1, false, false, false, allocs, 0)
	...
	_, split, _, _ = fm.BestSplitter(target, &cases, &[]int{0}, 1, nil, 1, false, false, false, allocs, 0)
	...
	_, split, _, _ = mediumfm.BestSplitter(f, &cases, &[]int{0}, 1, nil, 1, false, false, false, allocs, 0)
...
func TestBigCatFeature(t *testing.T) {
	...
	//split f by medium f should send 1 and 2 to one side, coded 6
	_, split, _, _ := bigfm.BestSplitter(boolf, &cases, &[]int{0}, 1, nil, 1, false, false, false, allocs, 0)
